...
- src/components/sections/coaching/ (CoachingHero, CoachingResonance, CoachingProgram, CoachingMethod, CoachingAbout, CoachingFoundingRound, CoachingClosingCTA, CoachingBoundary; CoachingAbout ist eine eigene Datei, kein About.tsx-Alias, weil der Text speziell auf das Programm zugeschnitten ist und von About.tsx abweicht)
- src/components/forms/ (AssessmentForm, LeadMagnetForm, ResultActions)
- src/components/ui/ (Header, Footer, FadeIn, Eyebrow, ListMark, StepList, Section, Container, Heading, Button, Card), src/components/seo/JsonLd.tsx
- src/content/journal/<slug>.mdx; src/lib/ (assessment-config.ts, journal.ts, supabase.ts, scroll.ts, anchors.ts)
...
- Anchor-Regel: Inpage-Sprungziele (Section-/section-id sowie interne href="#..."- bzw. href="/#..."-Links) laufen ausschließlich über benannte Konstanten in src/lib/anchors.ts, nie als wörtlicher String an ID-Definition und Link-Stelle getrennt gepflegt.
...
8. Architektur-Refactoring nach architektur-audit.md, Fahrplan in Abschnitt 3 dort: Auftrag 1 (UI-Primitives, Startseite) erledigt auf Branch refactor/primitives, noch nicht gemergt. Auftrag 2 (/breathwork auf die Primitives migrieren, Heading-Variante emphasis, BreathworkAbout.tsx-Entscheidung) erledigt auf Branch refactor/breathwork, noch nicht gemergt. Auftrag 3 (/coaching in sections/coaching/*.tsx extrahieren, StepList.tsx, Heading-Variante display-sub, lib/anchors.ts) erledigt auf Branch refactor/coaching, noch nicht gemergt. Offen: Auftrag 4 (Metadata/JSON-LD vereinheitlichen), Auftrag 5 (Header/Footer, restliche Seiten, verbleibende border-primary/N-Fundstellen auf --color-hairline).

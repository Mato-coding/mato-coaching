...
- src/components/ui/ (Header, Footer, FadeIn, Eyebrow, ListMark, Section, Container, Heading, Button, Card), src/components/seo/JsonLd.tsx
...
- Ausnahmen sind erlaubt, wenn eine Sektion nachweislich von der gemeinsamen Größe abweicht (z. B. CTA.tsx' und BreathworkClosingCTA.tsx' schmalere Textspalte max-w-2xl) — dann bleibt die betroffene Stelle bewusst außerhalb des Primitives, statt sie unbeabsichtigt zu verkleinern/verbreitern oder ein Primitive mit widersprüchlicher className zu überladen (Cascade-Reihenfolge bei zwei Utilities auf derselben CSS-Property ist nicht garantiert quellcode-Reihenfolge). Dieselbe Logik gilt für Textspalten, die enger als ihr umgebender Container sind, aber (anders als Container width="prose") nicht selbst zentriert sein dürfen, z. B. die Fließtext-Blöcke in den breathwork/*.tsx-Sections: dort bleibt die Breite als max-w-measure-Klasse (Token, kein Arbitrary-Value) direkt am Element, statt sie in ein zusätzliches, zentrierendes Container-Primitive zu verschachteln.
- Heading kennt neben section eine emphasis-Variante für Sektionen, die bewusst stärker betont werden sollen: auf Mobil identisch mit section, ab md eine Stufe größer (48px, --text-h2-emphasis-md). Aktuell einzige Verwendung: Cause.tsx auf der Startseite.
...
8. Architektur-Refactoring nach architektur-audit.md, Fahrplan in Abschnitt 3 dort: Auftrag 1 (UI-Primitives, Startseite) erledigt auf Branch refactor/primitives, noch nicht gemergt. Auftrag 2 (/breathwork auf die Primitives migrieren, Heading-Variante emphasis, BreathworkAbout.tsx-Entscheidung) erledigt auf Branch refactor/breathwork, noch nicht gemergt. Offen: Auftrag 3 (/coaching in sections/coaching/*.tsx extrahieren), Auftrag 4 (Metadata/JSON-LD vereinheitlichen), Auftrag 5 (Header/Footer, restliche Seiten, verbleibende border-primary/N-Fundstellen auf --color-hairline).

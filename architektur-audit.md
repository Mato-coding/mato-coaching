...
**Auftrag 2 von 5 (Branch `refactor/breathwork`, noch nicht auf main gemergt):** `/breathwork` vollständig auf dieselben Primitives migriert, `Heading` um Variante `emphasis` erweitert. Damit behoben bzw. adressiert:
- **Befund A3** (2.1, `BreathworkAbout.tsx` toter Code) und der zugehörige **Befund C** (2.7, toter Export): Datei gelöscht, keine Importer verloren. `/breathwork` verwendet weiterhin bewusst `About.tsx` als `BreathworkAbout`-Alias (Kandidat-3-Entscheidung: eigener Text bleibt zurückgestellt).
- **Cause.tsx' abweichende H2-Größe** (2.2, Teil der H2-Kette-Notiz; früher als bewusste Ausnahme in CLAUDE.md geführt): jetzt über die neue `Heading`-Variante `emphasis` verdrahtet statt als Ausnahme außerhalb der Skala. Auf Mobil dadurch bewusst kleiner (Angleichung an `section`), auf Desktop unverändert 48px.
- **H2-Kette** (2.2) und **Section-Wrapper/Container-Breiten** (2.2) für `/breathwork`: alle 8 Sections in `sections/breathwork/*.tsx` auf `Section`/`Container`/`Heading`/`Button`/`Eyebrow` umgestellt. Die Audio-Sektion wurde aus `breathwork/page.tsx` in eine eigene `BreathworkAudio.tsx` extrahiert (Anchor-ID `audio-reset` unverändert, `page.tsx` ist jetzt eine reine Kompositionsliste ohne inline JSX-Sections).
- **Befund A** (2.3, ungenutzte Type-Scale-Tokens): weitere `max-w-[68ch]`-Fundstellen in `/breathwork` durch die Token-Klasse `max-w-measure` ersetzt (13 Fundstellen aus dem Audit betrafen `/breathwork` zu 5); die verbleibenden nicht-zentrierten Textspalten nutzen bewusst weiterhin die reine Klasse statt des `Container`-Primitives (`Container` zentriert per `mx-auto` und würde die Spalte gegenüber der nicht-zentrierten Headline verschieben), siehe CLAUDE.md „Architektur-Konventionen".
- Impressum-E-Mail auf `hello@lassekluever.de` korrigiert (2.4 erwähnte diese Inkonsistenz am Rand). `datenschutz/page.tsx` hat dieselbe veraltete Adresse und wurde bewusst nicht angefasst (Struktur-Migration der Rechtsseiten folgt erst in Auftrag 4).

**Auftrag 3 von 5 (Branch `refactor/coaching`, noch nicht auf main gemergt):** `/coaching` nach dem `/breathwork`-Muster in `sections/coaching/*.tsx` extrahiert. Damit behoben bzw. adressiert:
- **Befund A2** (2.1, `/coaching` hat keine extrahierten Sections): alle 8 Sektionen (Hero, Resonanz, Programm, Methode, Über mich, Gründungsrunde, CTA, Abgrenzung) liegen jetzt als eigene Komponenten in `sections/coaching/*.tsx`, alle auf `Section`/`Container`/`Heading`/`Button`/`Eyebrow` umgestellt; `coaching/page.tsx` ist eine reine Kompositionsliste plus Metadata und unverändertem Inline-JSON-LD. Die Über-mich-Sektion wurde als eigene `CoachingAbout.tsx` extrahiert (kein `About.tsx`-Alias): der Text ist programmspezifisch und weicht wortlautlich von `About.tsx` ab.
- **Befund B2** (2.1, impliziter Cross-Page-Vertrag über die Anchor-ID): neues `src/lib/anchors.ts` mit benannten Konstanten (`AUDIO_ANCHOR`, `AUDIO_RESET_ANCHOR`, `PROGRAMM_ANCHOR`). ID-Definition (`LeadMagnet.tsx`, `BreathworkAudio.tsx`, `CoachingProgram.tsx`) und alle Link-Stellen (`BreathworkHero.tsx`, `CoachingHero.tsx`, `CoachingClosingCTA.tsx`) referenzieren dieselbe Konstante statt eines wörtlichen Strings. Der Cross-Page-Vertrag bleibt implizit bestehen (ein Import-Fehler würde weiterhin nicht auffallen, wenn die Section entfernt würde), ist jetzt aber an einer Stelle benannt und nicht mehr lautlos duplizierbar.
- **Schrittlisten-Duplikation** (2.2, „Nummerierte Schritt-Listen – zwei parallele, praktisch identische Implementierungen"): `BreathworkProcess.tsx`' `steps`-Rendering und `coaching`'s `bausteine`-Rendering waren strukturell identisch (`ListMark` + fettes Titelwort + Fließtext, `<ol>` mit `space-y-8`) und sind jetzt in einer gemeinsamen Komponente `StepList.tsx` (`src/components/ui/`) zusammengeführt.
- **Hero-H1-Größe**: `BreathworkHero.tsx`' und `coaching`'s Hero-H1 waren bereits deckungsgleich (`text-5xl md:text-6xl`, keine `lg`-Stufe), dafür jetzt eine eigene `Heading`-Variante `display-sub` (dieselben Tokens wie `display`, ohne `lg`-Sprung auf 72px). Die bisher nur als Code-Kommentar in `BreathworkHero.tsx` dokumentierte Ausnahme entfällt.
- Weiteres `max-w-[68ch]`-Vorkommen (2.3 Befund A) in den ehemals inline `coaching/page.tsx`-Sektionen (6 Fundstellen) durch `max-w-measure` ersetzt.

**Auftrag 4 von 5 (Branch `refactor/seo-pages`, noch nicht auf main gemergt):** Metadata und JSON-LD vereinheitlicht (Kandidat 4, **vollständig behoben**), restliche Seiten auf die UI-Primitives gestellt. Damit behoben bzw. adressiert:
- **Variante A/B-Redundanz** (2.6): `metadataBase`, Title-Template und Beschreibung standen byte-identisch in `app/layout.tsx` UND `(public)/layout.tsx`. Jetzt nur noch im Root-Layout; `(public)/layout.tsx` hat keinen eigenen `metadata`-Export mehr.
- **Vorschlag 2 aus 2.6** (Helper `buildMetadata`): umgesetzt in `src/lib/site.ts`, nimmt `{ path, title, description, ogType? }` entgegen und leitet Top-Level-Felder, `openGraph` und `alternates.canonical` (immer über `absoluteUrl()`) daraus ab. Alle Seiten (`/`, `/breathwork`, `/coaching`, `/assessment`, `/termin`, `/journal`, `/journal/[slug]`, `/impressum`, `/datenschutz`) nutzen ihn jetzt; die doppelt getippten title/description-Paare in Variante C entfallen. Startseite hat weiterhin eine eigens für Social-Previews formulierte, vom Seitentitel abweichende `openGraph`-Kopie (Bild, Locale, Siegename) — die war vorher im Public-Layout hinterlegt und ist jetzt direkt in `page.tsx` verortet, inhaltlich unverändert.
- **Variante D-Bruch** (2.6, relatives Canonical plus lokal redeklarierte `baseUrl`-Konstante): `journal/[slug]/page.tsx`s `generateMetadata` nutzt jetzt `buildMetadata()`, Canonical und `mainEntityOfPage` laufen über `absoluteUrl()`.
- **Variante E** (2.6, fehlendes Canonical/OpenGraph): `/assessment`, `/termin`, `/journal` hatten bisher gar keine eigene `alternates.canonical`/`openGraph` und erbten dadurch unbemerkt die Startseiten-spezifischen Werte aus dem Public-Layout (Canonical zeigte fälschlich auf `/`, OpenGraph-Titel war der Homepage-Titel). Jetzt über `buildMetadata()` korrekt pro Seite gesetzt, im gebauten HTML verifiziert.
- **JSON-LD-Boilerplate** (2.2, 4×): gemeinsame Primitive `src/components/seo/JsonLdScript.tsx`, alle vier Konsumenten (`JsonLd.tsx`, `breathwork/page.tsx`, `coaching/page.tsx`, `journal/[slug]/page.tsx`) darauf umgestellt. Schema-Inhalte unverändert. Entscheidung dokumentiert (jetzt auch in CLAUDE.md): das globale `ProfessionalService`-Schema bleibt bewusst auf jeder Seite (`JsonLd` im Public-Layout), Artikelseiten haben zusätzlich ihr eigenes `Article`-Schema — zwei JSON-LD-Blöcke pro Artikelseite sind korrekt und gewollt, keine Redundanz.
- **Rest von 2.2** (restliche Seiten auf die Primitives): `termin/page.tsx`, `journal/page.tsx`, `assessment/page.tsx`, `impressum/page.tsx`, `datenschutz/page.tsx` sowie die Eyebrow-Nachbauten in `AssessmentForm.tsx`/`AssessmentResult.tsx` auf `Section`/`Container`/`Eyebrow`/`Heading`/`Card`/`Button` umgestellt. Karten (ResultActions' drei Karten, Journal-Artikelkarten, Termin-Karte) laufen jetzt über `Card.tsx` (`border-hairline` statt `border-primary/8`–`/10`, minimal anderer Grauton, siehe design-system.md). `Card.tsx` um eine `compact`-Padding-Stufe (`p-4`) erweitert für die Termin-Karte. Impressum/Datenschutz wechseln von festem `py-32` auf `Section size="default"` (bewusste, im Auftrag freigegebene Verbesserung: reduziertes Mobil-Padding). Datenschutz-E-Mail auf `hello@lassekluever.de` korrigiert (letzte verbliebene Stelle mit der alten gmail-Adresse, Rest von Audit 2.4).
- Rendered-HTML-Diff gegen main (analog Auftrag 3): Seitentitel auf allen Seiten byte-identisch (inklusive der vorbestehenden, jetzt bewusst reproduzierten doppelten Title-Suffixe auf `/coaching`, `/impressum`, `/datenschutz` — Ursache war exakt die jetzt aufgelöste Layout-Redundanz, die Titel-Strings wurden 1:1 in `buildMetadata()` übernommen, damit sich am gerenderten Ergebnis nichts ändert). Canonical/OpenGraph-Diffs ausschließlich dort, wo der Auftrag sie ausdrücklich vorsah (neu für `/assessment`, `/termin`, `/journal`, `/impressum`, `/datenschutz`). `sitemap.xml` strukturell unverändert (nur Build-Zeitstempel).
- Kleine, außerhalb des engeren Auftragswortlauts liegende Normalisierung: `AssessmentResult.tsx`s Eyebrow-Abstand war hier `mb-8` (Sonderfall, Rest der Codebase durchgängig `mb-6`), die Migration auf `Eyebrow.tsx` übernimmt dessen festen Abstand `mb-6` statt den Sonderfall per `className`-Override zu erhalten (Cascade-Reihenfolge zweier Utilities auf derselben Property ist laut CLAUDE.md nicht garantiert) — sichtbarer Unterschied: 8px weniger Abstand zwischen Eyebrow und Ergebnis-Headline auf der Assessment-Ergebnisseite, zur visuellen Freigabe vorlegen.

**Auftrag 5 von 5 (Branch `refactor/cleanup`, letzter Auftrag der Serie, noch nicht auf main gemergt):** Typisierung, Journal-`draft`, Header-Client-Grenze, Rand-Töne, Tokens, Lint, Titel-Fix. Damit behoben bzw. adressiert:
- **Befund B, Typisierung** (2.7): `ResultActions`-Props `cluster`/`result` von `string` auf `Cluster`/`ResultRoute` aus `assessment-config.ts` umgestellt. `/api/assessment` validiert `cluster`/`route` jetzt gegen dieselben Union-Typen (400 bei ungültigem Wert statt beliebige Strings zu speichern) und prüft `answers` auf eine plausible Grobstruktur (Objekt, höchstens 20 Einträge, Werte String oder String-Array mit höchstens 20 Elementen) statt sie ungeprüft durchzureichen.
- **Befund B, `JournalMeta`/`draft`** (2.7): `JournalMeta` liest jetzt `updatedAt`, `excerpt`, `coverImage`, `tags` (Default `[]`) und `draft` (Default `false`) aus dem Frontmatter. `draft: true` wirkt: `getAllJournalEntries()` (Übersicht) und `getJournalSlugs()` (Sitemap, `generateStaticParams`, beide bauen jetzt auf `getAllJournalEntries()` auf statt auf ungefilterten Dateinamen) lassen den Artikel weg, `getJournalEntryBySlug()` liefert für einen draft-Artikel in Produktion (`NODE_ENV=production`) `null`, die Detailseite ruft dann `notFound()`; lokal und in Preview-Deployments bleibt der Artikel über die direkte URL ansehbar. Geprüft vor der Umsetzung: kein vorhandener Artikel hatte `draft: true` gesetzt (zwei der drei Artikel setzen `draft: false` explizit, einer hat das Feld gar nicht), also **keine sichtbare Verhaltensänderung** durch diesen Auftrag.
- **Befund B, `Header.tsx` Client-Grenze** (2.5): `Header.tsx` ist jetzt eine Server-Komponente, der Logo-Link (`usePathname` plus `smoothScrollToTop`) ist als eigene Client-Komponente `HeaderLogoLink.tsx` ausgelagert. Die beiden Logo-`<img>`-Tags laufen dabei über `next/image` (behebt zugleich die zwei `no-img-element`-Lint-Warnings aus Kriterium 8/Teil 5), mit `priority` (next/image preloadet das Bild dadurch automatisch wie zuvor der Browser-Heuristik-Preload für das im fixen Header immer sichtbare Logo — ohne `priority` wäre das Bild `loading="lazy"` geworden und der bisherige `<link rel="preload">` im `<head>` wäre entfallen, siehe Rendered-HTML-Diff) und `unoptimized` (SVG, next/image-Optimierung bringt hier nichts).
- **Befund B, Rand-Töne** (2.3): alle verbleibenden `border-primary/N`-Fundstellen auf `border-hairline` umgestellt: Header-Unterkante (`/5`), `About.tsx`' Credentials-Trennstrich (`/10`), `ResultActions.tsx`' toter „Bald verfügbar"-Zweig (`/10`, `JOURNAL_READY` ist `true`, dieser Zweig rendert aktuell nie), `LeadMagnetForm.tsx`- und `AssessmentForm.tsx`-Eingabefelder bzw. Antwort-Buttons (`/15`). Für die Eingabefeld-Frage geprüft: `--color-hairline` (muted bei 30% gegen Surface-Weiß) rendert zu ≈ `rgb(211,212,213)`, das bisherige `border-primary/15` (ink bei 15%) zu ≈ `rgb(220,220,221)` — `--color-hairline` ist also dunkler/kräftiger, nicht schwächer. Ein eigenes `--color-border-input`-Token war deshalb nicht nötig. Diese Berechnung ersetzt keine visuelle Prüfung im Browser; die Eingabefeld-Ränder stehen trotzdem auf der Liste zur visuellen Freigabe durch Lasse (siehe Qualitätssicherung unten).
- **Ungenutzte `@theme`-Tokens** (2.3 Befund A, Kandidat 6, offene Frage 6): grep-geprüft entfernt, weil ohne eine einzige Fundstelle im Code: `--text-h1`, `--text-body`, `--text-small`, `--text-eyebrow`, `--leading-h1`/`-body`/`-small`, `--opacity-hairline`, `--spacing-eyebrow-headline`/`-headline-body`, `--font-weight-regular`/`-semibold`/`-display`/`-hero`, `--duration-med`. Damit ist offene Frage 6 entschieden: unverdrahtete Richtwerte werden nicht länger als tote Dokumentation vorgehalten. `--font-weight-medium` sowie die numerische Spacing-Skala (`--spacing-1` bis `--spacing-40`) bleiben: Ersteres hat 14 Fundstellen (`font-medium`), Letztere sind zwar wertgleich mit Tailwinds eingebauter Default-Spacing-Skala, tragen aber als benannte Tokens den in design-system.md dokumentierten „Atem-Rhythmus" explizit im Code statt sich implizit auf Tailwinds Default zu verlassen.
- **Lint** (Kriterium 8): 5 vorbestehende Fehler behoben — `AssessmentForm.tsx`' Ref-Schreibzugriff während des Renderns (`autoScrolledRef.current = false` im selben Block wie ein Ref-Vergleich) durch den von React sanktionierten State-Vergleich im Render-Körper (State statt Ref für den Fragenwechsel-Vergleich) plus einen separaten, reinen Ref-Reset-Effekt ersetzt; `Method.tsx`, `Transformation.tsx`, `BreathworkResonance.tsx`' unescapte Anführungszeichen auf `&quot;` umgestellt (identisches Zeichen, keine Typografie-Änderung, um die Leitplanke „keine ungewollten visuellen Änderungen" strikt einzuhalten). Die 2 Header-`no-img-element`-Warnings über die `next/image`-Migration behoben (s. o.). `npm run lint` danach ohne Fehler und ohne Warnings.
- **Doppelte Title-Suffixe** (aus Auftrag 3/4 bewusst reproduziert, jetzt behoben): `/coaching`s Titel verlor das angehängte `| Lasse Klüver` (das Root-Layout-Title-Template hängt ohnehin `· Lasse Klüver` an), `/impressum` und `/datenschutz` bekamen kurze eigene Titel (`Impressum`, `Datenschutzerklärung`) statt des vollen Site-Default-Titels, der zuvor durch das Template ein zweites Mal denselben Suffix erhielt.
- Rendered-HTML-Diff gegen main (Prod-Build, zwei lokale `next start`-Instanzen, Normalisierung der hash-behafteten `_next/static`-Pfade und CSS-Modul-Klassennamen): einzige Abweichungen sind die drei genannten Titel, die Rand-Ton-Wechsel auf `border-hairline` und die durch `next/image` neu hinzugekommenen, rein strukturellen `<img>`-Attribute (`decoding="async"`, `data-nimg`, `style="color:transparent"` — kein Layout- oder Sichtbarkeitseffekt, `src`/`width`/`height` unverändert). `grep -rn "border-primary/[0-9]" src/` liefert keinen Treffer mehr.
- `npm run build` fehlerfrei, `npx tsc --noEmit` ohne Fehler.
...
## 5. Abschluss-Status (nach Auftrag 5 von 5)

Die fünfteilige Architektur-Refactoring-Serie ist abgeschlossen. Jeder Befund aus Abschnitt 2 und jeder Kandidat aus Abschnitt 3 ist entweder behoben oder als bewusste, dokumentierte Ausnahme markiert; jede offene Frage aus Abschnitt 4 ist durch die tatsächliche Umsetzung beantwortet.

**Befunde (Abschnitt 2), Status:**
- 2.1 Befund A1 (Doppel-FadeIn): behoben, Auftrag 1.
- 2.1 Befund A2 (`/coaching` ohne Sections): behoben, Auftrag 3.
- 2.1 Befund A3 / 2.7 Befund C (`BreathworkAbout.tsx` tot): behoben (Datei gelöscht), Auftrag 2. Bewusste Ausnahme bleibt bestehen: `/breathwork` nutzt weiterhin `About.tsx` als `BreathworkAbout`-Alias, ein eigener Text ist zurückgestellt (unverändert seit Auftrag 2).
- 2.1 Befund B2 (impliziter Cross-Page-Anchor-Vertrag): adressiert über `lib/anchors.ts`, Auftrag 3. Der Vertrag selbst bleibt bewusst implizit (siehe dortige Begründung), das ist keine offene Lücke, sondern eine bekannte Grenze benannter Konstanten.
- 2.2 Eyebrow-/Section-/Button-/H2-/Karten-/Schrittlisten-Wildwuchs: behoben, Aufträge 1–4 (Primitives Section/Container/Heading/Button/Card/Eyebrow/ListMark/StepList, durchgängig eingesetzt).
- 2.2 JSON-LD-Boilerplate: behoben über `JsonLdScript.tsx`, Auftrag 4.
- 2.3 Befund A (ungenutzte Type-Scale-/Layout-Tokens): `--text-display`/`-h2` und Containerbreiten verdrahtet (Auftrag 1), `max-w-[68ch]`-Fundstellen auf `max-w-measure` umgestellt (Auftrag 2/3), verbleibende unverdrahtete Tokens entfernt statt eingeführt (Auftrag 5, entscheidet offene Frage 6).
- 2.3 Befund B (`--color-hairline` nur an 3 Stellen): vollständig behoben, Auftrag 5 — keine `border-primary/N`-Fundstelle mehr im `src`-Baum.
- 2.3 Befund B (Section-Padding-Ausreißer): behoben. Coaching-Abgrenzungssektion und Impressum/Datenschutz laufen jetzt über `Section.tsx`s `size`-Stufen (`compact` bzw. `default`) statt eigener `py-*`-Werte; die Frage, ob design-system.md's Desktop-Spielraum bis 160px variieren soll, ist durch `Section.tsx`s drei feste, diskrete Stufen (`compact`/`default`/`spacious`) beantwortet — keine kontinuierliche Spanne, siehe design-system.md Abschnitt 3.
- 2.3 Befund C (`text-[--color-navy]`): behoben, Auftrag 1.
- 2.4 Copy-Verortung (Home/`/breathwork`/`/coaching` hartkodiert): bewusst **nicht** behoben, siehe Offene Aufgaben in CLAUDE.md — optionale, nicht priorisierte Extraktion, kein Teil der fünfteiligen Serie.
- 2.4 Impressum-E-Mail-Inkonsistenz: behoben, Auftrag 2 (`/breathwork`) und Auftrag 4 (`/datenschutz`).
- 2.5 Befund B (`Header.tsx` Client-Grenze): behoben, Auftrag 5.
- 2.6 Metadata/JSON-LD-Varianten A–E: behoben, Auftrag 4 (`buildMetadata()`, `JsonLdScript.tsx`).
- 2.6 doppelte Title-Suffixe (Nebenwirkung der Layout-Redundanz, in Auftrag 4 bewusst reproduziert dokumentiert): behoben, Auftrag 5.
- 2.7 Befund B (`ResultActions`-Typisierung, `/api/assessment`-Validierungstiefe): behoben, Auftrag 5.
- 2.7 Befund B (`JournalMeta`/`draft`): behoben, Auftrag 5. Kein Artikel hatte `draft: true` gesetzt, keine sichtbare Verhaltensänderung.
- 2.7 Befund C (JSON-LD-Objekte ohne gemeinsamen Typ): bewusst **nicht** behoben — kein funktionaler Fehler, kein Teil der fünfteiligen Serie, weiterhin offen für einen künftigen, eigenständigen Auftrag.

**Kandidaten (Abschnitt 3), Status:** 1–9 alle behoben (Aufträge 1–5), außer der optionalen Copy-Extraktion (2.4), die nie als eigener Kandidat gelistet war und bewusst außerhalb der Serie bleibt.

**Offene Fragen (Abschnitt 4), Antworten durch die Umsetzung:**
1. Entschieden gegen eine kontinuierliche Spanne: `Section.tsx` bietet drei feste Stufen (`compact`/`default`/`spacious`), design-system.md entsprechend präzisiert.
2. Beide Ausreißer an den Standard angeglichen (s. o.), keine bewusste Abweichung mehr.
3. `BreathworkAbout.tsx` gelöscht, `/breathwork` nutzt weiterhin den `About.tsx`-Alias (Text-Entscheidung bleibt zurückgestellt, keine Blockade für diese Serie).
4. `/coaching` dem `/breathwork`-Muster angeglichen (Auftrag 3).
5. Primitive-Umfang erweitert auf Heading und Card (Auftrag 1), vor der `/ifs`-Seite verfügbar.
6. Ungenutzte Type-Scale-Tokens entfernt statt eingeführt (Auftrag 5).
7. `draft` ist jetzt wirksam (Auftrag 5).

Verbleibend außerhalb dieser Serie, unverändert offen: die optionale Copy-Extraktion (2.4) und ein gemeinsamer Typ für JSON-LD-Objekte (2.7 Befund C), beide dokumentiert in CLAUDE.md bzw. hier als bewusst nicht priorisiert.
